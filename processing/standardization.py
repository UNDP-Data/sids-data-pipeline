...
def standardize(src_blob_path=None, dst_prj=4326, band=None,
    ...
    if alternative_path:
        assert os.path.exists(
            alternative_path), f'intermediary_folder={alternative_path} does not exist'
    ...
    if src_srs is None:
        ...
        src_srs.ImportFromWkt(src_ds.GetProjection()
                              or src_ds.GetProjectionRef())

    if src_srs is None:
        logger.warning(
            f'Assuming {src_blob_path} features EPSG:{dst_prj} projection')
        src_srs = osr.SpatialReference()
    ...
    try:
        ...
        proj_are_equal = int(src_srs.GetAuthorityCode(
            None)) == int(dst_srs.GetAuthorityCode(None))
    ...
    if should_reproject:
        ...
        src = gdal.Translate(
            destName=vrt_path, srcDS=src_ds, bandList=band_list,)

        res = gdal.Warp(destNameOrDestDS=dst_path,
                        srcDSOrSrcDSTab=src, options=options)
    else:
